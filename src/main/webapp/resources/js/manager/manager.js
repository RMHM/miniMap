		    ...
		    success : function(data){
				...
				for(var i in data){
					...
					if(mtype == "m"){
						...
						/*.append($tdAuthority)*/.append($('<td>').html("<button name='requestBtn'>권한확인</button>")).append($tdJoinDate).append($tdDropDate);	
						...
						if(data[i].dropDate == null && data[i].endDate > today){
							var $tdDropDate = $('<td>').text("정지");
							$tdDropDate.css('color', 'red');
						} else if (data[i].dropDate == null && data[i].endDate < today){
